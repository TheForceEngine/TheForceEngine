...
namespace TFE_Audio
{
	...
	{
		// Use the standard MIDI channel-mode messages (matching SystemMidiDevice) rather than
		// sending an explicit Note Off for every one of the 128 x 16 possible note/channel
		// combinations. The brute-force version previously used here queued 2048 events per
		// call, which could overflow the event queue (silently dropping legitimate note-off
		// events - audible as hanging/stuck notes) and spammed the log with dropped-event
		// warnings. All Sound Off (120) mutes immediately; All Notes Off (123) is the more
		// widely supported "release" variant - both are sent per channel for compatibility
		// with as many CLAP instruments as possible, and 32 total messages is negligible.
		for (u32 c = 0; c < MIDI_CHANNEL_COUNT; c++)
		{
			message(u8(MID_CONTROL_CHANGE + c), MID_ALL_SOUND_OFF, 0);
			message(u8(MID_CONTROL_CHANGE + c), MID_ALL_NOTES_OFF, 0);
		}
	}
